...
def data_cleaning(dir):  # input as a folder
    ...
    for file in filenames:
        df = pd.read_excel(
            file
        , sheet_name= None)  # making a dictionary where each key is related to the dataframe of each sheet
        keys = list(df.keys())
        for key in keys:    
            per_sheet = df[key]
        ...
        with pd.ExcelWriter('output.xlsx', engine='xlsxwriter') as writer: #you can change the file path here to whatever you want
...
def duplicate_analysis(per_sheet):  # of each sheet
    ...
    df['z_score'] = df.groupby('Metabolite Name')['Area'].apply(lambda x: (x - x.mean())/x.std())
    # all the NAN values are changed to zero
    df['z_score']= df['z_score'].fillna(0)
    # all rows whose z-score is less than 1.25 are kept
    df = df[abs(df['z_score']) < 1.25 ]
    # their mean is taken
    mean_df = df.groupby('Metabolite Name').agg( Area=('Area', 'mean'), n=('Metabolite Name', 'size')).reset_index()
    return mean_df
